...
export function getRecommendations(inputs, calculations, completeness) {
  ...
  const carKm = parseFloat(safeInputs.carKmPerWeek) || 0;
  const electricity = parseFloat(safeInputs.electricityKwhPerMonth) || 0;
  const meatMeals = parseFloat(safeInputs.meatMealsPerWeek) || 0;
  const purchases = parseFloat(safeInputs.purchasesPerMonth) || 0;
  const recycling = safeInputs.recyclingHabits || 'none';

  const cats = safeCalculations.categories || { transport: 0, electricity: 0, food: 0, shopping: 0, waste: 0 };
  const total = safeCalculations.totalMonthly || 1;
