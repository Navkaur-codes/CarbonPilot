...
export function validateAndSanitizeProfile(data) {
  ...
  const sanitizeNumField = (val, max) => {
    ...
    // Strip HTML and trim first
    const cleanStr = typeof val === 'string' ? val.replace(/<[^>]*>/g, '').trim() : String(val);
...
export function validateAndSanitizeGoals(data) {
  ...
  return data
    .map(goal => {
      ...
      const title = typeof goal.title === 'string' ? goal.title.replace(/<[^>]*>/g, '').trim() : '';
      const action = typeof goal.action === 'string' ? goal.action.replace(/<[^>]*>/g, '').trim() : '';
      const target = typeof goal.target === 'string' ? goal.target.replace(/<[^>]*>/g, '').trim() : '';
...
export function getStorageItem(key, defaultValue) {
  ...
  try {
    ...
    if (key === STORAGE_KEYS.PROFILE) {
      return validateAndSanitizeProfile(parsed);
    }
    if (key === STORAGE_KEYS.HISTORY) {
      return validateAndSanitizeHistory(parsed);
    }
    if (key === STORAGE_KEYS.STREAK) {
      return validateAndSanitizeStreak(parsed);
    }
    if (key === STORAGE_KEYS.GOALS) {
      return validateAndSanitizeGoals(parsed);
    }
